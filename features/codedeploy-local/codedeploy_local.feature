...
Feature: Local Deploy using AWS CodeDeploy Local CLI
  ...
  Scenario: Doing a sample local deployment using a directory bundle
    ...
    And the expected files (6) should have have been locally deployed to my host
  ...
  Scenario: Doing two sample local deployment using a directory bundle runs correct previous revision scripts
    ...
    Then the local deployment command should succeed
    And the expected files (6) should have have been locally deployed to my host twice
  ...
  Scenario: Doing a sample local deployment using a directory bundle with custom event
    ...
    Then the local deployment command should succeed
    And the expected files (2) should have have been locally deployed to my host
  ...
  Scenario: Doing a sample local deployment using a directory bundle without file-exists-behavior and existing file
    Given I have a sample local directory_with_destination_files bundle with custom appspec filename appspec_override_file_exists_behavior_disallow.yaml
    And I have existing file in destination
    When I create a local deployment with my bundle with file-exists-behavior MISSING
    Then the local deployment command should fail

  Scenario: Doing a sample local deployment using a directory bundle with file-exists-behavior OVERWRITE in appspec
    Given I have a sample local directory_with_destination_files bundle with custom appspec filename appspec_override_file_exists_behavior_overwrite.yaml
    And I have existing file in destination
    When I create a local deployment with my bundle
    Then the local deployment command should succeed
    And the expected existing file should end up like file-exists-behavior OVERWRITE specifies

  Scenario: Doing a sample local deployment using a directory bundle with file-exists-behavior RETAIN in appspec
    Given I have a sample local directory_with_destination_files bundle with custom appspec filename appspec_override_file_exists_behavior_retain.yaml
    And I have existing file in destination
    When I create a local deployment with my bundle
    Then the local deployment command should succeed
    And the expected existing file should end up like file-exists-behavior RETAIN specifies