#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────────
# End-to-end test for the Rust CodeDeploy agent on EC2 — IamUser mode.
#
# Launches an AL2023 EC2 instance, creates a dedicated IAM user, registers
# the instance as an on-premises instance with that user's ARN, then runs
# the agent on the instance with an `on_premises_config_file` that contains
# `iam_user_arn` + inline `aws_access_key_id` / `aws_secret_access_key`.
...
#
# Usage:
#   ./scripts/e2e-ec2-iam-user.sh setup     # Create all AWS resources + launch EC2
#   ./scripts/e2e-ec2-iam-user.sh run       # Connect via SSM and run agent (foreground)
#   ./scripts/e2e-ec2-iam-user.sh deploy    # Trigger a deployment
#   ./scripts/e2e-ec2-iam-user.sh status    # Check deployment status
#   ./scripts/e2e-ec2-iam-user.sh teardown  # Delete all AWS resources
#   ./scripts/e2e-ec2-iam-user.sh all       # setup + run (background) + deploy + status + teardown
#
# Prerequisites:
...
REMOTE_E2E_DIR="/tmp/codedeploy-agent-ec2-iam-user"
...
cmd_setup() {
    check_prereqs
    info "Setting up EC2 + IamUser e2e test resources in ${REGION}..."
    info "  Credential mode: IamUser (on-prem registration, NOT IMDS)"
    ...
    # 5. Register instance as on-prem so the agent uses IamUser credentials, not IMDS.
...
EOFCONFIG"
    # On-prem config — IamUser inline keys.
...
echo "DEPLOYMENT_ID=$DEPLOYMENT_ID"
echo "LIFECYCLE_EVENT=$LIFECYCLE_EVENT"
echo "Deployment successful! (EC2 + IamUser credential path)"
SCRIPT
    ...
    info ""
    info "Next steps:"
    info "  1. ./scripts/e2e-ec2-iam-user.sh run     # Start the agent on EC2 in foreground (SSM Session Manager)"
    info "  2. ./scripts/e2e-ec2-iam-user.sh deploy  # Trigger a deployment (in another terminal)"
    info "  3. ./scripts/e2e-ec2-iam-user.sh status  # Check deployment status"
    info "  4. ./scripts/e2e-ec2-iam-user.sh logs    # Tail the agent log"
}
...
cmd_deploy() {
    ...
    info "Watch progress: ./scripts/e2e-ec2-iam-user.sh status"
...
cmd_status() {
    ...
    if ! wait_for_deployments "$REGION_STATE" 600 "$DEPLOYMENT_ID"; then
        print_banner "TIMEOUT" "EC2 + IamUser E2E — deployment did not reach terminal state in 10 min"
        die "Timeout"
    fi
    ...
    if (( DEPLOY_SUCCEEDED == DEPLOY_TOTAL )); then
        print_banner "PASS" "EC2 + IamUser E2E — deployment Succeeded" \
            "Deployment: ${DEPLOYMENT_ID}" \
            "Credential mode: IamUser (on-prem registration, NOT IMDS)" \
            "Elapsed: ${DEPLOY_ELAPSED}s"
        print_deployment_outcome SUCCEEDED
    else
        print_banner "FAIL" "EC2 + IamUser E2E — deployment did not succeed" \
            "Deployment: ${DEPLOYMENT_ID}" \
...
# ── Teardown ────────────────────────────────────────────────────────
cmd_teardown() {
    info "Tearing down EC2 + IamUser test resources..."
    STATE=$(load_state)
...
case "${1:-help}" in
    ...
    help|*)
        ...
        echo ""
        echo "  Tests the IamUser credential path on an EC2 instance"
